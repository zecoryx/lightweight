...
# Global imports for all commands
...
class AgenticCLI:
    ...
    def _handle_slash(self, text: str):
        ...
        if cmd in ["/exit", "/quit", "/q"]:
            ...
            if not arg:
                console.print("[dim]  usage: /check <model_id> (e.g. llama-3-8b)[/dim]\n")
                return
            
            console.print(f"\n  [bold cyan]🔍 Checking performance for: {arg}[/bold cyan]")
            report = self.detector.get_report()
            est = StrategyEngine(report).estimate_performance(arg)
            ...
            table.add_row("Model Size", f"{est['original_gb']:.1f} GB", f"{est['compressed_gb']:.1f} GB")
            table.add_row("Speed", est['speed_before'], est['speed_after'])
            table.add_row("Quality Loss", "0%", est['quality'])
            table.add_row("Efficiency", "Max CPU/Heat", f"{est['threads']} Threads (Cool)")
            
            console.print(table)
            console.print(f"  [dim]Result: This model will run [white]{est['speed_after']}[/white] on your device.[/dim]\n")
    ...
    def run(self):
        ...
        session = PromptSession(
            ...
            style=PTStyle.from_dict({
                "bottom-toolbar": "bg:default fg:#888888",
                "prompt": "bg:#222222 fg:#ffffff bold",
                "": "bg:#222222 fg:#ffffff",
            }),
        )
        ...
        while self.is_running:
            try:
                # Clean prompt without extra padding or w-full block
                prompt_msg = HTML("<style fg='#ffffff'> › </style>")
...
def pull(
    ...
    quant: Optional[str] = typer.Option(None, "--quant", "-q", help="Manual quantization (e.g. Q4_K_M, IQ4_XS)")
):
...
@app.command()
def check(
    model_id: str = typer.Argument(..., help="Model ID to check (e.g. llama-3-8b)")
):
    """Estimate model performance on your hardware before downloading."""
    from lightweight.strategy import StrategyEngine
    from lightweight.hardware import Detector
    
    console.print(f"\n  [bold cyan]🔍 Checking performance for: {model_id}[/bold cyan]")
    detector = Detector()
    report = detector.get_report()
...
@app.command()
def serve(
    host: str = typer.Option("0.0.0.0", "--host", help="Binding host"),
    port: int = typer.Option(8000, "--port", "-p", help="Port to run the API server on")
):
    """Start the LightWeight API server (OpenAI compatible)."""
    import uvicorn
    console.print(f"\n  [bold green]✦ LightWeight API Server[/bold green]")
    console.print(f"  [dim]Hosting on http://{host}:{port}[/dim]")
    console.print(f"  [dim]OpenAI compatible: http://{host}:{port}/v1[/dim]\n")
    
    uvicorn.run("lightweight.api:app", host=host, port=port, log_level="info")
